...
class BeevenueResponse(Response):
    ...
    def push_medium(self, medium: SpindexedMedium) -> None:
...
def _thumb_url(medium: SpindexedMedium) -> str:
...
def _file_url(medium: SpindexedMedium) -> str:
    """Get URL for this medium's file."""
    path = Path(
        "/",
        current_app.config.get("BEEVENUE_ROOT", "/"),
        "files",
        f"{medium.hash}.{EXTENSIONS[medium.mime_type]}",
    )
    return str(path)
