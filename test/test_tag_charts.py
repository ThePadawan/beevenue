...
def test_can_get_tag_similarity_chart_as_user(userClient):
    res = userClient.get("/tags/similarity")
    assert res.status_code == 200
    assert "links" in res.get_json()
    assert "nodes" in res.get_json()
...
def test_can_get_tag_implications_chart_as_user(userClient):
    res = userClient.get("/tags/implications")
    assert res.status_code == 200
    assert "links" in res.get_json()
    assert "nodes" in res.get_json()
